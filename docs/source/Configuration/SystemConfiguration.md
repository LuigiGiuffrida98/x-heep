...
The Python script should include a `config()` function that takes no parameters and returns a {py:class}`xheep_gen.xheep.XHeep` instance. An example is shown in [configs/general.py](https://github.com/x-heep/x-heep/blob/main/configs/general.py).
