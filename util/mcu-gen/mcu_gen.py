...
import xheep_gen.load_config
from xheep_gen.xheep import BusType
...
def generate_xheep(args):
    ...
    if args.python_config != None and args.python_config != "":
        xheep = xheep_gen.load_config.load_cfg_file(
            pathlib.PurePath(str(args.python_config))
        )
    else:
        xheep = xheep_gen.load_config.load_cfg_file(pathlib.PurePath(str(args.config)))
    ...
    pad_ring = xheep_gen.load_config.load_pad_cfg(pathlib.PurePath(str(args.pads_cfg)))
    if pad_ring is None:
