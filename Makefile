...
# ============================================================================
# Register Generation (incremental: each target rebuilds only when its .hjson changes)
# ============================================================================

# Peripherals whose .hjson is generated by mcu-gen; targets depend on $(MCU_GEN_STAMP).
#   REG_GEN_PERIPHERALS_MCU_FULL  — RTL + SW header + structs + docs (hw/ip/<name>/)
#   REG_GEN_PERIPHERALS_MCU_BASIC — RTL + SW header + docs, no structs (hw/ip/<name>/)
REG_GEN_PERIPHERALS_MCU_FULL  ?= soc_ctrl power_manager pdm2pcm
REG_GEN_PERIPHERALS_MCU_BASIC ?=

# Macro: RTL + SW header + SW structs + docs for hw/ip/<name>/ peripherals
# $(1) = peripheral name, $(2) = extra prerequisites
...
# Macro: RTL + SW header + docs, no structs (hw/ip/<name>/)
# $(1) = peripheral name, $(2) = extra prerequisites
...
# Macro: FULL for peripherals outside hw/ip/ (custom base path)
# $(1) = peripheral name, $(2) = base dir (has data/ and rtl/ subdirs), $(3) = extra prerequisites
define PERIPH_GEN_FULL_AT

$(2)/rtl/$(1)_reg_top.sv: $(2)/data/$(1).hjson | $(3)
	@mkdir -p $(2)/rtl
	@printf -- "Generating $(1) RTL..."; \
	 $(PYTHON) $(REGTOOL) -r -t $(2)/rtl $$< && printf " OK\n"

sw/device/lib/drivers/$(1)/$(1)_regs.h: $(2)/data/$(1).hjson | $(3)
	@mkdir -p sw/device/lib/drivers/$(1)
	@printf -- "Generating $(1) SW header..."; \
	 $(PYTHON) $(REGTOOL) --cdefines -o $$@ $$< && printf " OK\n"

sw/device/lib/drivers/$(1)/$(1)_structs.h: $(2)/data/$(1).hjson | $(3)
	@mkdir -p sw/device/lib/drivers/$(1)
	@printf -- "Generating $(1) SW structs..."; \
	 $(PYTHON) $(PERIPH_STRUCTS_GEN) --template_filename $(TEMPLATE_FILE) \
	   --hjson_filename $$< --output_filename $$@ && printf " OK\n"

sw/device/lib/drivers/$(1)/$(1)_regs.md: $(2)/data/$(1).hjson | $(3)
	@mkdir -p sw/device/lib/drivers/$(1)
	@printf -- "Generating $(1) docs..."; \
	 $(PYTHON) $(REGTOOL) -d $$< > $$@ && printf " OK\n"
...
REG_GEN_OUTPUTS :=
$(foreach p,$(REG_GEN_PERIPHERALS_MCU_FULL),$(eval $(call PERIPH_GEN_FULL,$(p),$(MCU_GEN_STAMP))))
$(foreach p,$(REG_GEN_PERIPHERALS_MCU_BASIC),$(eval $(call PERIPH_GEN_BASIC,$(p),$(MCU_GEN_STAMP))))
$(eval $(call PERIPH_GEN_FULL_AT,pad_control,hw/system/pad_control,$(MCU_GEN_STAMP)))
$(eval $(call PERIPH_GEN_FULL_AT,dma,hw/vendor/xheep/dma,$(MCU_GEN_STAMP)))

# Boot ROM: file target keyed on assembly sources; just `all` (no clean) for incremental build.
# Note: does NOT track sw/device/lib/drivers/**/*_regs.h; run `make boot-rom` if headers changed.
BOOT_ROM_SOURCES := hw/ip/boot_rom/boot_rom.S hw/ip/boot_rom/link.ld hw/ip/boot_rom/gen_rom.py

hw/ip/boot_rom/boot_rom.sv: $(BOOT_ROM_SOURCES)
	@$(MAKE) -C hw/ip/boot_rom all

## Force full boot ROM rebuild (use after changing driver register headers)
.PHONY: boot-rom
boot-rom:
	@$(MAKE) -C hw/ip/boot_rom clean all

# SPI: primary output file as target; sub-Makefile regenerates all spi outputs as side effects.
SPI_PRIMARY := hw/vendor/xheep/spi/rtl/w25q128jw_controller/rtl/w25q128jw_controller_reg_top.sv

$(SPI_PRIMARY): hw/vendor/xheep/spi/rtl/w25q128jw_controller/data/w25q128jw_controller.hjson
...
.PHONY: reg-gen
reg-gen: $(REG_GEN_OUTPUTS) hw/ip/boot_rom/boot_rom.sv $(SPI_PRIMARY)
...
verilator-build: $(MCU_GEN_STAMP) $(REG_GEN_OUTPUTS) hw/ip/boot_rom/boot_rom.sv $(SPI_PRIMARY) | .check-verilator
...
verilator-build-sc: $(MCU_GEN_STAMP) $(REG_GEN_OUTPUTS) hw/ip/boot_rom/boot_rom.sv $(SPI_PRIMARY) | .check-verilator
...
questasim-build: $(MCU_GEN_STAMP) $(REG_GEN_OUTPUTS) hw/ip/boot_rom/boot_rom.sv $(SPI_PRIMARY)
...
## @param CPU=cv32e20(default),cv32e40p,cv32e40x,cv32e40px
## @param BUS=onetoM(default),NtoM
vcs-build: $(MCU_GEN_STAMP) $(REG_GEN_OUTPUTS) hw/ip/boot_rom/boot_rom.sv $(SPI_PRIMARY)
...
## VCS-AMS simulation:
vcs-ams-build: $(MCU_GEN_STAMP) $(REG_GEN_OUTPUTS) hw/ip/boot_rom/boot_rom.sv $(SPI_PRIMARY)
...
## xcelium simulation
xcelium-build: $(MCU_GEN_STAMP) $(REG_GEN_OUTPUTS) hw/ip/boot_rom/boot_rom.sv $(SPI_PRIMARY)
...
clean: clean-app
	@rm -rf $(BUILD_DIR)
	@rm -f $(REG_GEN_OUTPUTS) $(MCU_GEN_STAMP)
	@$(MAKE) -C hw/ip/boot_rom clean
